...
public class UserDaoImpl implements UserDao {
	...
	public ArrayList<User> login(User u) {
		...
		try {
			...
			if (rs.first()) {
				...
				System.out.println("userdaoImpl,user:"+refreshList.get(0).toString());
				return refreshList;
			}
		} catch (SQLException e) {
			 e.printStackTrace();
	...
	public User findMe(int id) {
		...
		System.out.println("findMe, id:"+id);
		...
		try {
			...
			ResultSet rs = ps.executeQuery();
			if (rs.first()) {
				System.out.println("findMe, rs is not null");
	...
	public ArrayList<User> refresh(int id) {
		...
		User me = findMe(id);
		System.out.println("userdaoImpl,refresh,me:"+me.toString());
		list.add(me);// 先添加自己
		Connection con = DButil.connect();
